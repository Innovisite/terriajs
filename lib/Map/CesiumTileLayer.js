            ...
            if (!this._delayedUpdate) {
                this._delayedUpdate = setTimeout(function() {
                    ...
                    if (!(tilingScheme instanceof WebMercatorTilingScheme)) {
                        that.errorEvent.raiseEvent(that, 'Cet ensemble de données ne peut être affiché sur la carte 2D car il n\'est pas compatible avec la projection Web Mercator (EPSG:3857)');
                        return;
                    }
                    ...
                    if (tilingScheme.getNumberOfXTilesAtLevel(0) === 2 && tilingScheme.getNumberOfYTilesAtLevel(0) === 2) {
                        ...
                        that.errorEvent.raiseEvent(that, 'Cet ensemble de données ne peut être affiché sur la carte 2D car il utilise un schéma de tuiles particulier non supporté.');
                        return;
                    }
