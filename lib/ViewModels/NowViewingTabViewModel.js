...
var NowViewingTabViewModel = function(options) {
    ...
    this.name = defaultValue(options.name, 'Affichage');
    this.nowViewing = options.nowViewing;
    this.badgeText = this.nowViewing.items.length === 0 ? undefined : this.nowViewing.items.length;
    this.noDataText = defaultValue(options.noDataText, "Ajoutez des données depuis le Catalogue.");
...
NowViewingTabViewModel.prototype.dragStart = function(viewModel, e) {
    ...
    if (defined(e.dataTransfer)) {
        e.dataTransfer.setData('text', 'Déplacez l\'élément visualisé.');
    } else {
        e.originalEvent.dataTransfer.setData('text', 'Déplacez l\'élément visualisé.');
    }
