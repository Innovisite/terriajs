...
var NowViewingTabViewModel = function(options) {
    ExplorerTabViewModel.call(this, defaultValue(options.name, '@<<NOW_VIEWING_NAME>>@'), defaultValue(options.name, 'NowViewing'));
    this.nowViewing = options.nowViewing;
    this.badgeText = this.nowViewing.items.length === 0 ? undefined : this.nowViewing.items.length;
    this.noDataText = defaultValue(options.noDataText, '@<<NOW_VIEWING_NODATA>>@');
...
NowViewingTabViewModel.prototype.dragStart = function(viewModel, e) {
    ...
    if (defined(e.dataTransfer)) {
        e.dataTransfer.setData('text', '@<<NOW_VIEWING_MOVE_ITEM>>@');
    } else {
        e.originalEvent.dataTransfer.setData('text', '@<<NOW_VIEWING_MOVE_ITEM>>@');
    }
