...
ToolsPanelViewModel.prototype.show = function(container) {
    this._domNodes = loadView(require('../Views/ToolsPanel.html'), container, this);
};
...
function requestTiles(toolsPanel, requests, minLevel, maxLevel) {
    ...
    function doneUrl(stat, startTime, error, maxAverage) {
    ...
    function doNext() {
        ...
        loadWithXhr({
            ...
            doneUrl(next.stat, start, false, maxAverage);
            ...
            doneUrl(next.stat, start, true, maxAverage);
        });
    }
    ...
    var sendNext = function(currentIndex) {
        ...
        when(requests[currentIndex].item._cache()).then(function(){
            if (defined(requests[currentIndex].item._postCache)) {
                requests[currentIndex].item._postCache();
            }
            if (requests[currentIndex].enabledHere) {
                requests[currentIndex].item._disable();
            }
