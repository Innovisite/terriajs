...
var FeatureInfoPanelViewModel = function(options) {
    ...
    this.name = defaultValue(options.name, '@<<FEATURE_INFO_TITLE>>@');
    ...
    this._domNodes = loadView(require('../Views/FeatureInfoPanel.html'), container, this);
    ...
    if (defined(this.terria.pickedFeatures)) {
        this.showFeatures(this.terria.pickedFeatures);
    }
...
FeatureInfoPanelViewModel.prototype.showFeatures = function(features) {
    ...
    return when(features.allFeaturesAvailablePromise, function() {
        ...
        var featuresWithInfo = features.features.filter(featureHasInfo);

        if (featuresWithInfo.length === 0) {
            that.resetNameAndMessage(undefined, '@<<FEATURE_NONE>>@');
            that.isVisible = that.showPanelWhenNoFeatures;
            that.terria.selectedFeature = undefined;
            return;
        }
        that.resetNameAndMessage();
        that.resetSections();
        if (featuresWithInfo.indexOf(that.terria.selectedFeature) < 0) {
            that.terria.selectedFeature = featuresWithInfo[0];
        }
