...
NominatimSearchProviderViewModel.prototype.search = function(searchText) {
    ...
    var promiseBounded = loadJson(this.url + 'search?q=' + searchText + bboxStr +  
				  '&bounded=1&format=json&countrycodes=fr&limit=' 
				  + this.limitBounded);
    var promiseOthers = loadJson(this.url + 'search?q=' + searchText +
				 '&format=json&countrycodes=fr&limit=' + this.limitOthers);

    var that = this;
    var geocodeInProgress = this._geocodeInProgress = when.all([ promiseBounded, promiseOthers ]).then(function(result) {
	...
	var findDbl = function(elts, id) {
	    return elts.find(function(elt) {
		return elt.id === id ? true : undefined;
	    });
	};
