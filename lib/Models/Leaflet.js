 ...
 */
Leaflet.prototype.captureScreenshot = function(type, ratio) {
    ...
    try {
        html2canvas(this.map.getContainer(), {
			...
			var curRatio = canvas.width / canvas.height;
			var cx = 0, cy = 0;
			var cw = canvas.width;
			var ch = canvas.height;
			if(curRatio > ratio) { // crop the width
			    cx = Math.round(cw / 2);
			    cw = Math.round(canvas.height * ratio);
			    cx -= Math.round(cw / 2);
			    
			} else { // crop the height
			    cy = Math.round(ch / 2);
			    ch = Math.round(canvas.width / ratio);
			    cy -= Math.round(ch / 2);
			}
			...
			dataUrl = dstCanvas.toDataURL(type);
		    }
                } catch (e) {
                    deferred.reject(e);
                }
