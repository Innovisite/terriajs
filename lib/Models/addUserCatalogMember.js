...
var addUserCatalogMember = function(terria, newCatalogMemberOrPromise, options) {
    ...
    return when(newCatalogMemberOrPromise, function(newCatalogItem) {
        ...
        if (!(e instanceof ModelError)) {
            e = new ModelError({
                title: 'Les données ne peuvent être ajoutées',
                message: 'Les données spécifiées ne peuvent être ajoutées parce ce qu\'elles sont invalides ou le format attendu n\'est pas respecté.'
            });
        }
