 ...
 */
Cesium.prototype.captureScreenshot = function(type, ratio) {
    var deferred = when.defer();

    type = defined(type) ? type : 'image/png';

    var removeCallback = this.scene.postRender.addEventListener(function() {
        removeCallback();
        try {
	    if(!defined(ratio)) {
		deferred.resolve(this.scene.canvas.toDataURL(type));
	    } else {
		var curRatio = this.scene.canvas.width / this.scene.canvas.height;
		var cx = 0, cy = 0;
		var cw = this.scene.canvas.width;
		var ch = this.scene.canvas.height;
		if(curRatio > ratio) { // crop the width
		    cx = cw / 2;
		    cw = this.scene.canvas.height * ratio;
		    cx -= cw / 2;
		    
		} else { // crop the height
		    cy = ch / 2;
		    ch = this.scene.canvas.width / ratio;
		    cy -= ch / 2;
		}
		...
		ctx.drawImage(this.scene.canvas, cx, cy, cw, ch, 0, 0, cw, ch);
		deferred.resolve(dstCanvas.toDataURL(type));
	    }
        } catch (e) {
            deferred.reject(e);
        }
...
Cesium.prototype.addImageryLayer = function(options) {
    var scene = this.scene;
    scene.imageryLayers.add(options.layer);
};
