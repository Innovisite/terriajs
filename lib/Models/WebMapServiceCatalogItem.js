...
var WebMapServiceCatalogItem = function(terria) {
    ...
    /**
     * Gets or sets additional parameters to pass to the WMS server when requesting
     * layer information
     * @type {String}
     */
    ...
    overrideProperty(this, 'legendUrl', {
        get : function() {
            ...
            return new LegendUrl(cleanUrl(this.url) + '?service=WMS&version=1.3.0&request=GetLegendGraphic&format=image/png&layer=' + layer + 
		(defined(this.extra) ? '&' + this.extra : ''), 'image/png');
        },
        set : function(value) {
...
function updateIntervalsFromTimes(result, times, index) {
    ...
    for (var i = 0; i < dimensions.length; ++i) {
        ...
        for (var j = 0; j < times.length; ++j) {
            ...
            if (isoSegments.length > 1) {
                ...
                updateIntervalsFromTimes(result, times, j);
            }
        }
    }
